...
> Let $F$ be a finite family of $d$-element subsets of $\{1,\ldots,n\}$, closed under replacing $q>1$ by $q-1$ whenever $q$ belongs to a subset and $q-1$ does not. Let $e_r$ count the members of $F$ containing $r$, with $e_{n+1}=0$, and let $\Delta_r$ be the leading principal minor of order $r$ of an $n\times n$ matrix $A$ over a commutative ring. Then
> $$
